...
! level-set discretization option): here the user
...
#include "mmg/mmg3d/libmmg3df.h"
  ...
  !> 3) Build the level-set and metric in MMG5 format
  ...
  !> load the level-set with MMG3D_loadSol function
  ...
  !    is tensorial
  zero_ikind = 0
  CALL MMG3D_Get_meshSize(mmgMesh,np,%val(zero_ikind),%val(zero_ikind),&
       %val(zero_ikind),%val(zero_ikind),%val(zero_ikind),ier)
  IF ( ier /= 1 ) THEN
     CALL EXIT(204)
  ENDIF


  CALL MMG3D_Set_solSize(mmgMesh,mmgMet,MMG5_Vertex,np,MMG5_Tensor,ier)
  IF ( ier /= 1 ) THEN
     CALL EXIT(304)
  ENDIF
  ...
  DO k=1,np
     ...
     CALL MMG3D_Set_tensorSol(mmgMet,10.D0,0.D0,0.D0,1.D0,0.D0,1.D0,k,ier)
     IF ( ier /= 1 ) THEN
        CALL EXIT(404)
     ENDIF
  ENDDO
  ...
  CALL MMG3D_mmg3dls(mmgMesh,mmgLs,mmgMet,ier)
  ...
  CALL MMG3D_saveSol(mmgMesh,mmgMet,TRIM(ADJUSTL(outname)),&
       LEN(TRIM(ADJUSTL(outname))),ier)
  IF ( ier /= 1 ) THEN
     CALL EXIT(107)
  ENDIF
