...
#include "mmg/mmgs/libmmgsf.h"
  ...
  !> load the level-set with MMGS_loadSol function
  ...
  !    is tensorial
  zero_ikind = 0
  CALL MMGS_Get_meshSize(mmgMesh,np,zero_ikind,zero_ikind,ier)
  IF ( ier /= 1 ) THEN
     CALL EXIT(204)
  ENDIF


  CALL MMGS_Set_solSize(mmgMesh,mmgMet,MMG5_Vertex,np,MMG5_Tensor,ier)
  IF ( ier /= 1 ) THEN
     CALL EXIT(304)
  ENDIF
  ...
  DO k=1,np
     ...
     CALL MMGS_Set_tensorSol(mmgMet,10000.D0,0.D0,0.D0,100.D0,0.D0,1.D0,k,ier)
     IF ( ier /= 1 ) THEN
        CALL EXIT(404)
     ENDIF
  ENDDO
