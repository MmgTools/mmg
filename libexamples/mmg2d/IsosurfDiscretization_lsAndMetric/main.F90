...
#include "mmg/mmg2d/libmmg2df.h"
  ...
  !> load the level-set with MMG2D_loadSol function
  ...
  !    is tensorial
  CALL MMG2D_Get_meshSize(mmgMesh,np,%val(0_immg),%val(0_immg),%val(0_immg),ier)
  IF ( ier /= 1 ) THEN
     CALL EXIT(204)
  ENDIF


  CALL MMG2D_Set_solSize(mmgMesh,mmgMet,MMG5_Vertex,np,MMG5_Tensor,ier)
  IF ( ier /= 1 ) THEN
     CALL EXIT(304)
  ENDIF
  ...
  DO k=1,np
     ...
     CALL MMG2D_Set_tensorSol(mmgMet,10000.D0,0.D0,100.D0,k,ier)
     IF ( ier /= 1 ) THEN
        CALL EXIT(404)
     ENDIF
  ENDDO
  ...
  CALL MMG2D_mmg2dls(mmgMesh,mmgLs,mmgMet,ier)
