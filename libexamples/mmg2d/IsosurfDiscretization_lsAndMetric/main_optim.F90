...
#include "mmg/mmg2d/libmmg2df.h"
  ...
  CALL MMG2D_Set_iparameter(mmgMesh,0_8,MMG2D_IPARAM_iso, 1,ier)
  IF ( ier == 0 )  CALL EXIT(101)

  ! Ask for optim mode: compute the mean of input edge lengths.
  CALL MMG2D_Set_iparameter(mmgMesh,0_8,MMG2D_IPARAM_optim, 1,ier)
  IF ( ier == 0 )  CALL EXIT(101)

  ! Ask to do this with anisotropic metric (unit tensor metric is computed).
  CALL MMG2D_Set_iparameter(mmgMesh,0_8,MMG2D_IPARAM_anisosize, 1,ier)
  IF ( ier == 0 )  CALL EXIT(101)
