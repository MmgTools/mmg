...
/**
 * Example of use of the mmg2dls function of the mmg2d library: here the user
...
int main(int argc,char *argv[]) {
  ...
  if ( MMG2D_Set_iparameter(mmgMesh,mmgLs,MMG2D_IPARAM_iso, 1) != 1 )
    exit(EXIT_FAILURE);
  ...
  /** load the level-set with MMG2D_loadMesh function */
  ...
  for(k=1 ; k<=np ; k++) {
    ...
    if ( MMG2D_Set_tensorSol(mmgMet,10000,0,100,k) != 1 ) exit(EXIT_FAILURE);
  }
  ...
  ier = MMG2D_mmg2dls(mmgMesh,mmgLs,mmgMet);
  ...
  if ( MMG2D_saveSol(mmgMesh,mmgMet,outname) != 1 )
    exit(EXIT_FAILURE);
