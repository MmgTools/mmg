...
int chkcol(MMG5_pMesh mesh,MMG5_pSol met,int k,char i,int *list,char typchk) {
  ...
  ip2 = pt->v[i2];
  if ( typchk == 2 && met->m ) {
    lon = _MMG5_lenedg(mesh,met,ip1,ip2,0,0);
  ...
  if ( ilist > 3 ) {
    ...
    for (l=1; l<ilist-1+open; l++) {
      ...
      if ( typchk == 2 && met->m && !MG_EDG(mesh->point[ip2].tag) ) {
        ip1 = pt1->v[j2];
        len = _MMG5_lenedg(mesh,met,ip1,ip2,0,0);
        if ( len > lon )  return(0);
      }
      ...
      /* check quality */
      if ( typchk == 2 && met->m )
        kal = ALPHAD*_MMG5_calelt(mesh,met,pt0,0);
      else
        kal = ALPHAD*_MMG5_caltri_iso(mesh,NULL,pt0,0);
