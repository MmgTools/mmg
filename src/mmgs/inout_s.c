    ...
    if( mesh->info.iso ) {
      ...
      for (k=1; k<=na; k++) {
        if (!bin) {
          ...
          MMG_FREAD(&a,sw,1,inm);
          if(iswp) a=swapbin(a);
          MMG_FREAD(&b,sw,1,inm);
          if(iswp) b=swapbin(b);
      ...
      for (k=1; k<=mesh->na; k++) {
        if (!bin) {
          ...
          MMG_FREAD(&mesh->edge[k].a,sw,1,inm);
          if(iswp) mesh->edge[k].a=swapbin(mesh->edge[k].a);
          MMG_FREAD(&mesh->edge[k].b,sw,1,inm);
