...
static int MMGS_plyHeader(FILE *inm,MMGS_PlyHeader *header) {
  ...
  while ( fgets(line,sizeof(line),inm) ) {
    ...
    if ( !strcmp(word,"format") ) {
      ...
      long long count;
      if ( sscanf(line,"element %63s %lld",name,&count) != 2 || count < 0 ) return 0;
      if ( sizeof(MMG5_int) == 4 && count > INT32_MAX ) return 0;
...
static int MMGS_plyReadValue(FILE *inm,int binary,enum MMGS_PlyType type,
  ...
  if ( !binary ) {
    ...
    if ( type == MMGS_PLY_F32 || type == MMGS_PLY_F64 ) {
      *real = strtod(token,&end); *integer = (int64_t)*real;
    }
    else {
      *integer = strtoll(token,&end,10); *real = (double)*integer;
    }
  ...
  case MMGS_PLY_F32: { uint32_t b=(uint32_t)bits; float f; memcpy(&f,&b,4); *real=f; *integer=(int64_t)f; return isfinite(*real); }
  case MMGS_PLY_F64: { double d; memcpy(&d,&bits,8); *real=d; *integer=(int64_t)d; return isfinite(*real); }
  default: return 0;
  }
  *real = (double)*integer;
  return 1;
...
static int MMGS_plyAppendTriangle(MMGS_PlyTriangles *triangles,
  ...
  if ( triangles->size == triangles->capacity ) {
    size_t old = triangles->capacity,newCapacity = old ? 2*old : 256;
...
int MMGS_loadPlyMesh(MMG5_pMesh mesh,const char *filename) {
  ...
  MMG5_SAFE_MALLOC(points,3*header.np,double,goto memory_error);
  ...
  for ( i=0; i<header.nface; ++i ) {
    ...
    for ( j=0; j<header.nFaceProperties; ++j ) {
      if ( header.face[j].isList ) {
        ...
        if ( sizeof(MMG5_int) == 4 && integer > INT32_MAX ) goto parse_error;
        count=(MMG5_int)integer;
        if ( j==indexProperty && (size_t)count>polygonCapacity ) {
          size_t old=polygonCapacity;
          MMG5_SAFE_REALLOC(polygon,old,count,MMG5_int,"PLY polygon",goto memory_error);
          polygonCapacity=count;
        }
        for ( k=0; k<count; ++k ) {
        ...
        if ( j==refProperty ) {
          if ( sizeof(MMG5_int) == 4 &&
               (integer < INT32_MIN || integer > INT32_MAX) ) goto parse_error;
    ...
    for ( j=1; j<polygonSize-1; ++j ) {
      if ( !MMGS_plyAppendTriangle(&triangles,polygon[0],polygon[j],polygon[j+1],ref) ) goto memory_error;
    }
  }
  if ( !MMGS_Set_meshSize(mesh,header.np,(MMG5_int)triangles.size,0) ) goto memory_error;
  for ( i=0; i<header.np; ++i ) if ( !MMGS_Set_vertex(mesh,points[3*i],points[3*i+1],points[3*i+2],0,i+1) ) goto parse_error;
  for ( i=0; i<(MMG5_int)triangles.size; ++i ) if ( !MMGS_Set_triangle(mesh,triangles.vertices[3*i],triangles.vertices[3*i+1],triangles.vertices[3*i+2],triangles.refs[i],i+1) ) goto parse_error;
...
static int MMGS_plyWriteDouble(FILE *out,double value) {
  uint64_t bits; memcpy(&bits,&value,8); return MMGS_plyWriteBytes(out,bits,8);
}

int MMGS_savePlyMesh(MMG5_pMesh mesh,const char *filename) {
  MMG5_pPoint point; MMG5_pTria triangle;
  MMG5_int i,np=0,nt=0; const char *extension; FILE *out; int ascii;
  if ( !filename ) return 0;
  extension=strrchr(filename,'.'); ascii=extension && !strcmp(extension,".plya");
  if ( !(out=fopen(filename,ascii?"w":"wb")) ) return 0;
  for ( i=1; i<=mesh->np; ++i ) { point=&mesh->point[i]; point->tmp=MG_VOK(point)?++np:0; }
  for ( i=1; i<=mesh->nt; ++i ) {
    triangle = &mesh->tria[i];
    if ( MG_EOK(triangle) && mesh->point[triangle->v[0]].tmp &&
         mesh->point[triangle->v[1]].tmp &&
         mesh->point[triangle->v[2]].tmp ) ++nt;
  }
  ...
  for ( i=1; i<=mesh->np; ++i ) { point=&mesh->point[i]; if (!point->tmp) continue;
    if ( ascii ) fprintf(out,"%.17g %.17g %.17g\n",point->c[0],point->c[1],point->c[2]);
    else if ( !MMGS_plyWriteDouble(out,point->c[0]) || !MMGS_plyWriteDouble(out,point->c[1]) || !MMGS_plyWriteDouble(out,point->c[2]) ) goto error;
  }
  for ( i=1; i<=mesh->nt; ++i ) { triangle=&mesh->tria[i];
    if ( !MG_EOK(triangle) || !mesh->point[triangle->v[0]].tmp ||
         !mesh->point[triangle->v[1]].tmp ||
         !mesh->point[triangle->v[2]].tmp ) continue;
    if ( ascii ) fprintf(out,"3 %" MMG5_PRId " %" MMG5_PRId " %" MMG5_PRId " %" MMG5_PRId "\n",mesh->point[triangle->v[0]].tmp-1,mesh->point[triangle->v[1]].tmp-1,mesh->point[triangle->v[2]].tmp-1,triangle->ref);
    else if ( !MMGS_plyWriteBytes(out,3,1) || !MMGS_plyWriteBytes(out,(uint32_t)(mesh->point[triangle->v[0]].tmp-1),4) || !MMGS_plyWriteBytes(out,(uint32_t)(mesh->point[triangle->v[1]].tmp-1),4) || !MMGS_plyWriteBytes(out,(uint32_t)(mesh->point[triangle->v[2]].tmp-1),4) || !MMGS_plyWriteBytes(out,(uint32_t)triangle->ref,4) ) goto error;
  }
  return !fclose(out);
error: fclose(out); return 0;
}
