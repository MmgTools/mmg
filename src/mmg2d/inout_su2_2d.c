...
static int MMG2D_su2MarkerRef(const char *name,MMG5_int *reference,
...
static int MMG2D_su2CompareRef(const void *left,const void *right) {
...
/** Assign positive fallback references after every explicit reference is
 * known.  Sorting the reserved values avoids quadratic behavior for files
 * with many boundary markers. */
static int MMG2D_su2ResolveMarkerRefs(MMG5_int *refs,
  ...
  qsort(reserved,(size_t)nreserved,sizeof(MMG5_int),MMG2D_su2CompareRef);
  for ( i=0; i<nmark; ++i ) {
...
int MMG2D_loadSu2Mesh(MMG5_pMesh mesh,const char *filename) {
  ...
  MMG5_int      *edges = NULL,*edgeRefs = NULL,*markerRefs = NULL;
  MMG5_int      *reservedRefs = NULL;
  ...
  if ( status > 0 ) {
    ...
    for ( i=0; i<nmark; ++i ) {
      ...
      tag[sizeof(tag)-1] = '\0';
      if ( !MMG2D_su2MarkerRef(tag,&markerRefs[i],&explicitRefs[i]) ||
      ...
      for ( j=0; j<markerElements; ++j ) {
        ...
        /* Store the marker ordinal until explicit references from every
         * marker are known and collision-free fallbacks can be assigned. */
        edgeRefs[na++] = i;
      }
    }
    if ( !MMG2D_su2ResolveMarkerRefs(markerRefs,explicitRefs,reservedRefs,
                                     nmark) ) goto parse_error;
    for ( i=0; i<na; ++i ) edgeRefs[i] = markerRefs[edgeRefs[i]];
  }
  for ( i=0; i<nelem; ++i ) {
