 ...
 * \warning Not used.
 **/
int MMG5_chkptonbdy(MMG5_pMesh mesh,MMG_int np){
  MMG5_pTetra      pt;
  MMG5_pxTetra     pxt;
  MMG5_pPoint      p0;
  MMG_int              k;
...
int MMG5_chkfemtopo(MMG5_pMesh mesh) {
  ...
  MMG5_pxTetra     pxt;
  MMG5_pPoint      p0,p1;
  MMG_int          k,ntet,ned,np,list[MMG3D_LMAX+2],np1,npchk,iel;
 ...
 * \warning Not used.
 */
int srcface(MMG5_pMesh mesh,MMG_int n0,MMG_int n1,MMG_int n2) {
  MMG5_pTetra   pt;
  MMG5_pxTetra  pxt;
  int           ref;
  MMG_int       minn,maxn,sn,k,ip0,ip1,ip2,mins,maxs,sum;
  int16_t       tag;
  int8_t        i;
  static int8_t mmgWarn0 = 0;
