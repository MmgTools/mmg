...
int MMG3D_loadMesh_opened(MMG5_pMesh mesh,FILE *inm,int bin) {
  ...
  int         i,bdim,binch,ref,iswp;
  MMG5_int    *ina,v[3],nt,na,nr,ia,aux,nref;
  ...
  if (!bin) {
    ...
    while(fscanf(inm,"%127s",&chaine[0])!=EOF && strncmp(chaine,"End",strlen("End")) ) {
      ...
      if(!strncmp(chaine,"MeshVersionFormatted",strlen("MeshVersionFormatted"))) {
        MMG_FSCANF(inm,"%" MMG5_PRId "",&mesh->ver);
        continue;
      } else if(!strncmp(chaine,"Dimension",strlen("Dimension"))) {
        MMG_FSCANF(inm,"%" MMG5_PRId "",&mesh->dim);
        if(mesh->dim!=3) {
          fprintf(stderr,"BAD DIMENSION : %" MMG5_PRId "\n",mesh->dim);
          return -1;
        }
        continue;
    ...
    while(fread(&binch,MMG5_SW,1,inm)!=0 && binch!=54 ) {
      ...
      if(!bdim && binch==3) {  //Dimension
        ...
        mesh->dim = bdim;
        if(bdim!=3) {
          fprintf(stderr,"BAD MESH DIMENSION : %" MMG5_PRId "\n",mesh->dim);
  ...
  if ( mesh->nt ) {
    ...
    if( mesh->info.iso ) {
      mesh->nt = 0;
      MMG5_SAFE_CALLOC(ina,nt+1,MMG5_int,return -1);

      for (k=1; k<=nt; k++) {
        if (!bin) {
          MMG_FSCANF(inm,"%" MMG5_PRId " %" MMG5_PRId " %" MMG5_PRId " %" MMG5_PRId "",&v[0],&v[1],&v[2],&ref);
        }
        else {
          for (i=0 ; i<3 ; i++) {
...
int MMG3D_loadMshMesh(MMG5_pMesh mesh,MMG5_pSol sol,const char *filename) {
  ...
  if ( nsols>1 ) {
    fprintf(stderr,"SEVERAL SOLUTION => IGNORED: %" MMG5_PRId "\n",nsols);
    fclose(inm);
...
int MMG3D_loadAllSols(MMG5_pMesh mesh,MMG5_pSol *sol, const char *filename) {
  ...
  if ( nsols > MMG5_NSOLS_MAX ) {
    fprintf(stderr,"\n  ## Error: %s: unexpected number of data (%" MMG5_PRId ").\n",
            __func__,nsols);
    MMG5_SAFE_FREE(type);
    fclose(inm);
    return -1;
