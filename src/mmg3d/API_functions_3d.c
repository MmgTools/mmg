...
int MMG3D_Set_solSize(MMG5_pMesh mesh, MMG5_pSol sol, int typEntity, int np, int typSol) {
  ...
  if ( ( (mesh->info.imprim > 5) || mesh->info.ddebug ) && sol->m )
    fprintf(stdout,"  ## Warning: %s: old solution deletion.\n",__func__);

  if ( typEntity != MMG5_Vertex ) {
    fprintf(stderr,"  ## Error: %s: mmg3d need a solution imposed on vertices\n",
            __func__);
    return(0);
  }
  if ( typSol == MMG5_Scalar ) {
    ...
    fprintf(stderr,"  ## Error: %s: type of solution not yet implemented\n",
            __func__);
    return(0);
  }
...
int MMG3D_Set_meshSize(MMG5_pMesh mesh, int np, int ne, int nprism,
  ...
  if ( ( (mesh->info.imprim > 5) || mesh->info.ddebug ) &&
    ...
    fprintf(stdout,"  ## Warning: %s: old mesh deletion.\n",__func__);
...
int MMG3D_Set_vertex(MMG5_pMesh mesh, double c0, double c1, double c2, int ref, int pos) {
  ...
  if ( !mesh->np ) {
    fprintf(stderr,"  ## Error: %s: you must set the number of points with the",
            __func__);
  ...
  if ( pos > mesh->npmax ) {
    fprintf(stderr,"  ## Error: %s: unable to allocate a new point.\n",__func__);
  ...
  if ( pos > mesh->np ) {
    fprintf(stderr,"  ## Error: %s: attempt to set new vertex at position %d.",
            __func__,pos);
...
int MMG3D_Get_vertex(MMG5_pMesh mesh, double* c0, double* c1, double* c2, int* ref,
  ...
  if ( mesh->npi == mesh->np ) {
    mesh->npi = 0;
    if ( mesh->info.ddebug ) {
      fprintf(stdout,"  ## Warning: %s: reset the internal counter of points.\n",
        __func__);
...
int MMG3D_Set_tetrahedron(MMG5_pMesh mesh, int v0, int v1, int v2, int v3, int ref, int pos) {
  ...
  if ( pos > mesh->ne ) {
    fprintf(stderr,"  ## Error: %s: attempt to set new tetrahedron at position %d.",
            __func__,pos);
  ...
  if ( vol <= _MMG5_EPSD2 ) {
    fprintf(stderr,"  ## Error: %s: tetrahedron %d has volume null.\n",
            __func__,pos);
    for ( ip=0; ip<4; ip++ ) {
...
int MMG3D_Get_tetrahedron(MMG5_pMesh mesh, int* v0, int* v1, int* v2, int* v3,
  ...
  if ( mesh->nei == mesh->ne ) {
    mesh->nei = 0;
    if ( mesh->info.ddebug ) {
      fprintf(stdout,"  ## Warning: %s: reset the internal counter of"
              " tetrahedra.\n",__func__);
      fprintf(stdout,"     You must pass here exactly one time (the first time ");
  ...
  if ( mesh->nei > mesh->ne ) {
    fprintf(stderr,"  ## Error: %s: unable to get tetra.\n",__func__);
...
int MMG3D_Set_prism(MMG5_pMesh mesh, int v0, int v1, int v2,
  ...
  if ( !mesh->nprism ) {
    fprintf(stderr,"  ## Error: %s: You must set the number of prisms with the",
      __func__);
    fprintf(stderr," MMG3D_Set_meshSize function before setting elements in mesh\n");
    return(0);
  }
  ...
  if ( pos > mesh->nprism ) {
    fprintf(stderr,"  ## Error: %s: attempt to set new prism at position %d.",
            __func__,pos);
...
int MMG3D_Get_prism(MMG5_pMesh mesh, int* v0, int* v1, int* v2, int* v3,
  ...
  if ( npri == mesh->nprism ) {
    npri = 0;
    if ( mesh->info.ddebug ) {
      fprintf(stdout,"  ## Warning: %s: reset the internal counter of prisms.\n",
              __func__);
      fprintf(stdout,"     You must pass here exactly one time (the first time ");
  ...
  if ( npri > mesh->nprism ) {
    fprintf(stderr,"  ## Error: %s: unable to get prism.\n",__func__);
...
int MMG3D_Set_triangle(MMG5_pMesh mesh, int v0, int v1, int v2, int ref,int pos) {
  ...
  if ( !mesh->nt ) {
    fprintf(stderr,"  ## Error: %s: You must set the number of triangles"
            " with the",__func__);
  ...
  if ( pos > mesh->ntmax ) {
    fprintf(stderr,"  ## Error: %s: unable to allocate a new triangle.\n",
            __func__);
  ...
  if ( pos > mesh->nt ) {
    fprintf(stderr,"  ## Error: %s: attempt to set new triangle at"
...
int MMG3D_Get_triangle(MMG5_pMesh mesh, int* v0, int* v1, int* v2, int* ref
  ...
  if ( mesh->nti > mesh->nt ) {
    fprintf(stderr,"  ## Error: %s: unable to get triangle.\n",__func__);
...
int MMG3D_Set_quadrilateral(MMG5_pMesh mesh, int v0, int v1, int v2, int v3,
  ...
  if ( !mesh->nquad ) {
    fprintf(stderr,"  ## Error: %s: You must set the number of quadrilaterals"
            " with the",__func__);
  ...
  if ( pos > mesh->nquad ) {
    fprintf(stderr,"  ## Error: %s: attempt to set new quadrilateral"
...
int MMG3D_Get_quadrilateral(MMG5_pMesh mesh, int* v0, int* v1, int* v2, int* v3,
  ...
  if ( nqi == mesh->nquad ) {
    nqi = 0;
    if ( mesh->info.ddebug ) {
      fprintf(stdout,"  ## Warning: %s: reset the internal counter"
              " of quadrilaterals.\n",__func__);
      fprintf(stdout,"     You must pass here exactly one time (the first time ");
  ...
  if ( nqi > mesh->nquad ) {
    fprintf(stderr,"  ## Error: %s: unable to get quadrilateral.\n",__func__);
...
int MMG3D_Set_edge(MMG5_pMesh mesh, int v0, int v1, int ref, int pos) {
  ...
  if ( !mesh->na ) {
    fprintf(stderr,"  ## Error: %s: You must set the number of edges with"
            " the",__func__);
    fprintf(stderr," MMG3D_Set_meshSize function before setting edges in mesh\n");
    return(0);
  }
  if ( pos > mesh->namax ) {
    fprintf(stderr,"  ## Error: %s: unable to allocate a new edge.\n",
      __func__);
  ...
  if ( pos > mesh->na ) {
    fprintf(stderr,"  ## Error: %s: attempt to set new edge at position %d.",
            __func__,pos);
...
int MMG3D_Get_edge(MMG5_pMesh mesh, int* e0, int* e1, int* ref
  ...
  if ( mesh->nai == mesh->na ) {
    mesh->nai = 0;
    if ( mesh->info.ddebug ) {
      fprintf(stdout,"  ## Warning: %s: reset the internal counter of edges.\n",
        __func__);
      fprintf(stdout,"     You must pass here exactly one time (the first time ");
  ...
  if ( mesh->nai > mesh->na ) {
    fprintf(stderr,"  ## Error: %s: unable to get edge.\n",__func__);
...
int MMG3D_Set_scalarSol(MMG5_pSol met, double s, int pos) {
  ...
  if ( !met->np ) {
    fprintf(stderr,"  ## Error: %s: You must set the number of"
  ...
  if ( pos < 1 ) {
    fprintf(stderr,"  ## Error: %s: unable to set a new solution.\n",__func__);
  ...
  if ( pos > met->np ) {
    fprintf(stderr,"  ## Error: %s: attempt to set new solution at"
            " position %d.",__func__,pos);
...
int MMG3D_Get_scalarSol(MMG5_pSol met, double* s) {
  ...
  if ( met->npi > met->np ) {
    fprintf(stderr,"  ## Error: %s: unable to get solution.\n",__func__);
...
int MMG3D_Set_scalarSols(MMG5_pSol met, double *s ) {
  ...
  if ( !met->np ) {
    fprintf(stderr,"  ## Error: %s: You must set the number of solution"
            " with the",__func__);
    fprintf(stderr," MMG3D_Set_solSize function before setting values");
    fprintf(stderr," in solution structure \n");
    return(0);
...
int MMG3D_Chk_meshData(MMG5_pMesh mesh,MMG5_pSol met) {
  ...
  if ( (mesh->npi != mesh->np) || (mesh->nei != mesh->ne) ) {
    fprintf(stderr,"  ## Error: %s: if you don't use the MMG3D_loadMesh"
            " function,",__func__);
  ...
  if ( met->npi != met->np ) {
    fprintf(stderr,"  ## Error: %s: if you don't use the MMG3D_loadSol"
            " function,",__func__);
...
int _MMG3D_skipIso(MMG5_pMesh mesh) {
  ...
  if ( (mesh->info.imprim > 5) || mesh->info.ddebug )
    fprintf(stdout,"  ## Warning: %s: skip of all entites with %d reference.\n",
            __func__,MG_ISO);
...
int MMG3D_Set_iparameter(MMG5_pMesh mesh, MMG5_pSol sol, int iparam,int val){
  ...
  case MMG3D_IPARAM_mem :
    if ( val <= 0 ) {
      fprintf(stdout,"  ## Warning: %s: maximal memory authorized must be"
              " strictly positive.\n",__func__);
  ...
  case MMG3D_IPARAM_angle :
    ...
    if ( !val )
      ...
      if ( (mesh->info.imprim > 5) || mesh->info.ddebug )
        fprintf(stdout,"  ## Warning: %s: angle detection parameter set"
                " to default value\n",__func__);
  ...
  case MMG3D_IPARAM_numberOfLocalParam :
    if ( mesh->info.par ) {
      ...
      if ( (mesh->info.imprim > 5) || mesh->info.ddebug )
        fprintf(stdout,"  ## Warning: %s: new local parameter values\n",__func__);
    }
    mesh->info.npar   = val;
    mesh->info.npari  = 0;
    mesh->info.parTyp = 0;
  ...
  default :
    fprintf(stderr,"  ## Error: %s: unknown type of parameter\n",__func__);
    return(0);
  }
  /* other options */
...
int MMG3D_Set_dparameter(MMG5_pMesh mesh, MMG5_pSol sol, int dparam, double val){
  ...
  case MMG3D_DPARAM_hausd :
    if ( val <=0 ) {
      fprintf(stderr,"  ## Error: %s: hausdorff number must be strictly"
              " positive.\n",__func__);
      return(0);
    }
    else
...
int MMG3D_Set_localParameter(MMG5_pMesh mesh,MMG5_pSol sol, int typ, int ref,
  ...
  if ( !mesh->info.npar ) {
    fprintf(stderr,"  ## Error: %s: You must set the number of local"
            " parameters",__func__);
  ...
  if ( mesh->info.npari >= mesh->info.npar ) {
    fprintf(stderr,"  ## Error: %s: unable to set a new local parameter.\n",
            __func__);
    fprintf(stderr,"    max number of local parameters: %d\n",mesh->info.npar);
    return(0);
  }
  if ( typ != MMG5_Triangle && typ != MMG5_Tetrahedron ) {
    fprintf(stderr,"  ## Warning: %s: you must apply your local parameters",
      __func__);
  ...
  if ( ref < 0 ) {
    fprintf(stderr,"  ## Error: %s: negative references are not allowed.\n",
      __func__);
    return(0);
  }
  ...
  for (k=0; k<mesh->info.npari; k++) {
    ...
    if ( par->elt == typ && par->ref == ref ) {
      ...
      par->hmax  = hmax;
      if ( (mesh->info.imprim > 5) || mesh->info.ddebug ) {
          fprintf(stdout,"  ## Warning: %s: new parameters (hausd, hmin and hmax)",
            __func__);
