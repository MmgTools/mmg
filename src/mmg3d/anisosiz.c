 ...
 * an "isotropic way" (we don't consider the metric direction, thus, we applied
 * the gradation on the metric size only).
 *
 */
static inline
int _MMG3D_grad2metSurf(MMG5_pMesh mesh, MMG5_pSol met, MMG5_pTria pt, int i){
  MMG5_pPoint   p1,p2;
  double   *mm1,*mm2,*nn1,*nn2,ps1_n,ps2_n,ps1_t,ps2_t,ux,uy,uz,m1[6],m2[6];
  ...
  if( MG_SIN(p1->tag) || (MG_NOM & p1->tag)){
    ...
    ps1_t = nt[0]*nn1[0] + nt[1]*nn1[1] + nt[2]*nn1[2];
    ps2_t = nt[0]*nn2[0] + nt[1]*nn2[1] + nt[2]*nn2[2];

    if( fabs(ps1_t) < fabs(ps2_t))
  ...
  /* ps1_t = ux*n1[0] + uy*n1[1] + uz*n1[2]; */
  /* t1[0] = ux - ps1_t*n1[0]; */
  /* t1[1] = uy - ps1_t*n1[1]; */
  /* t1[2] = uz - ps1_t*n1[2]; */

  /* // warning : t2 seems to be wrong calculated */
  /* ps2_t = - (ux*n2[0] + uy*n2[1] + uz*n2[2]); */
  /* t2[0] = -ux - ps1_t*n2[0]; */
  /* t2[1] = -uy - ps1_t*n2[1]; */
  /* t2[2] = -uz - ps1_t*n2[2]; */
  ...
  ps1_t =  mtan1[0]*t1[0]*t1[0] + 2.0*mtan1[1]*t1[0]*t1[1] + mtan1[2]*t1[1]*t1[1];
  //ps1_t += 2*mr1[2]*t1[0]*t1[2] + 2*mr1[4]*t1[1]*t1[2] + mr1[5]*t1[2]*t1[2];
  ...
  ps2_t = mtan2[0]*t2[0]*t2[0] + 2.0*mtan2[1]*t2[0]*t2[1] + mtan2[2]*t2[1]*t2[1];
  //ps2_t += 2*mr2[2]*t2[0]*t2[2] + 2*mr2[4]*t2[1]*t2[2] + mr2[5]*t2[2]*t2[2];
  ...
  if( ps2_t > ps1_t ){
    /* compute alpha = h2 + hgrad*l */
    alpha = ps2_t /(1.0+mesh->info.hgrad*l*ps2_t);
    if( ps1_t >= alpha -_MMG5_EPS )
      return(-1);
    ...
    /* Compute beta coef such as lambda_1 = beta*lambda_1 => h1 = h2 + hgrad*l
     * (see p317 of Charles Dapogny Thesis). */
    beta = (alpha*alpha - ps1_t*ps1_t)/(c[ichg]*c[ichg]);
  ...
  /* Metric in p2 has to be changed */
    ...
    alpha = ps1_t /(1.0+mesh->info.hgrad*l*ps1_t);
    if( ps2_t >= alpha - _MMG5_EPS)
      return(-1);
    ...
    /* Compute beta coef such as lambda_1 = beta*lambda_1 => h1 = h2 + hgrad*l
    * (see p317 of Charles Dapogny Thesis). */
    beta = (alpha*alpha - ps2_t*ps2_t)/(c[ichg]*c[ichg]);
 ...
 */
/* static inline */
/* int _MMG3D_grad2metSurfn(MMG5_pMesh mesh, MMG5_pSol met, MMG5_pTria pt, int i){ */
/*   MMG5_pPoint   p1,p2; */
/*   double   *mm1,*mm2,*nn1,*nn2,ps1_n,ps2_n,ps1_t,ps2_t,ux,uy,uz,m1[6],m2[6]; */
...
/*     /\* Project the vector t1 along the main directions of the metric *\/ */
/*     // Remark: the eigenv function reorder the eigenvectors, thus we don't know */
/*     // which on corresponds to the normal direction. */
/*     c[0] = t1[0]*vp[0][0] + t1[1]*vp[0][1] + t1[2]*vp[0][2]; */
/*     c[1] = t1[0]*vp[1][0] + t1[1]*vp[1][1] + t1[2]*vp[1][2]; */
/*     c[2] = t1[0]*vp[2][0] + t1[1]*vp[2][1] + t1[2]*vp[2][2]; */
...
/*       c[3] = fabs(m1[3]-lambda[ichg]); */
/*       c[4] = fabs(m1[4]-lambda[ichg]); */
...
/*     } */
/*     else if( p2->tag & MG_GEO ){ */
/*       c[0] = fabs(m2[0]-lambda[ichg]); */
/*       c[1] = fabs(m2[1]-lambda[ichg]); */
/*       c[2] = fabs(m2[2]-lambda[ichg]); */
/*       /\* c[3] = fabs(m2[3]-lambda[ichg]); *\/ */
/*       /\* c[4] = fabs(m2[4]-lambda[ichg]); *\/ */
        ...
        for (i=0; i<4; i++) {
          if ( pxt->ftag[i] & MG_BDY) {
            ...
            for (j=0; j<3; j++) {
              ...
              ier = _MMG3D_grad2metSurf(mesh,met,&ptt,j);
              if ( ier == i0 ) {
                p0->flag = mesh->base;
                nu++;
              }
             ...
             /* gradation in the normal direction of the tangent plane.*/
              /* ier = _MMG3D_grad2metSurfn(mesh,met,&ptt,j); */
              /* if ( ier == i0 ) { */
              /*   p0->flag = mesh->base; */
              /*   nu++; */
              /* } */
