        ...
        if ( typchk == 1 ) {
          ...
          if ( pt->xt )
            ll = _MMG5_lenedg(mesh,met,pt->v[ip],pt->v[iq],
                              (pxt->tag[_MMG5_iarf[i][j]] & MG_GEO),0);
          else
            ll = _MMG5_lenedg(mesh,met,pt->v[ip],pt->v[iq],0,0);
    ...
    for (i=0; i<6; i++) {
      ...
      if ( (p1->tag & MG_BDY) && (p2->tag & MG_BDY) ) {
        ...
        if (typchk == 1) {
          ...
          ll = _MMG5_lenedg(mesh,met,ip1,ip2,0,0);
  ...
  for (k=1; k<=mesh->ne; k++) {
    ...
    if ( typchk == 1 ) {
      ...
      for (j=0; j<3; j++) {
        ...
        ip1 = pt->v[i1];
        ip2 = pt->v[i2];
        len = _MMG5_lenedg(mesh,met,ip1,ip2, (pxt->tag[ia] & MG_GEO),0);
...
static int _MMG5_anatet4(MMG5_pMesh mesh, MMG5_pSol met) {
  ...
  for (k=1; k<=mesh->ne; k++) {
    ...
    if ( nf > 1 ) {
      if ( !_MMG5_split4bar(mesh,met,k) ) return(-1);
      ns++;
    }
    else {
      nf = 0;
...
int _MMG5_anatet(MMG5_pMesh mesh,MMG5_pSol met,char typchk, int patternMode) {
  ...
  do {
    ...
    if ( !mesh->info.noswap ) {
      ...
      nf = _MMG5_swptet(mesh,met,1.1,NULL);
      if ( nf < 0 ) {
        fprintf(stdout,"  ## Unable to improve mesh. Exiting.\n");
        return(0);
      }
