...
int chkswpbdy(pMesh mesh, int *list, int ilist, int it1, int it2){
  ...
  pPoint   p0,p1,ppt0;
  Tria     tt1,tt2;
  double   b0[3],b1[3],v[3],c[3],ux,uy,uz,disnat,dischg,cal1,cal2,calnat,calchg,calold,calnew;
  int      ref,iel,iel1,iel2,np,nq,na1,na2,k;
  char     tag,iface1,iface2,ia,ip,iq,ia1,ia2,j,isshell;
  ...
  if( (ref > 0) || MG_EDG(tag) || MG_SIN(tag) )
    return(0);
  ...
  tet2tri(mesh,iel1,iface1,&tt1);  
  tet2tri(mesh,iel2,iface2,&tt2); 

  for(ia1 = 0; ia1 < 3; ia1++){
    if((tt1.v[ia1] != np)&&(tt1.v[ia1] != nq))
      break;
  } 
  assert(ia1 < 3);
  na1 = tt1.v[ia1];
  assert((tt1.v[inxt2[ia1]] == np && tt1.v[iprv2[ia1]] == nq)||(tt1.v[inxt2[ia1]] == nq && tt1.v[iprv2[ia1]] == np)); // to be removed eventually
  
  for(ia2 = 0; ia2 < 3; ia2++){
    if((tt2.v[ia2] != np)&&(tt2.v[ia2] != nq))
      break;
  } 
  assert(ia2 < 3);
  na2 = tt2.v[ia2];
  assert((tt2.v[inxt2[ia2]] == np && tt2.v[iprv2[ia2]] == nq)||(tt2.v[inxt2[ia2]] == nq && tt2.v[iprv2[ia2]] == np)); // to be removed eventually

  /* Compare contributions to Hausdorff distance in both configurations */ 
  norface(mesh,iel1,iface1,v);
  ...
  if( dischg > info.hausd * info.hausd) return(0);
  if(( dischg < info.hausd * info.hausd)&&( disnat < info.hausd * info.hausd)){
    cal1 = caleltsurf(mesh,&tt1);
    cal2 = caleltsurf(mesh,&tt2);
  ...
  for (k=0; k<ilist; k++) {
    ...
    if ( !isshell ) {            
      ...
      //if ( orcal(mesh,0) < 0.001 )  return(0);
    }
    memcpy(pt0,pt,sizeof(Tetra)); 
...
void swpbdy(pMesh mesh, int *list, int ret, int it1){
  ...
  assert(colver(mesh,list,ilist,ipa));
}
