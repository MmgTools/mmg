...
int newElt(pMesh mesh) {
  ...
  if(MG_EOK(pt)) printf("On realloue %d qui est VALIDE !!!! \n",curiel);
  assert(!MG_EOK(pt));
...
void delElt(pMesh mesh,int iel) {
  ...
  mesh->nenil = iel;
	if ( iel == mesh->ne ) {
  // On degage des elements en faisant comme ça, et du coup, je me demande si on ne perd pas des pointeurs en 'sautant' comme ca.... 
