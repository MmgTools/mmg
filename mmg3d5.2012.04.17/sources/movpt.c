...
int movintpt(pMesh mesh,int *list,int ilist) {
  ...
  double     vol,totvol,calold,calnew;
  int        k,iel,i0;
  ...
  for (k=0; k<ilist; k++) {
    ...
    vol= fabs(det4pt(p0->c,p1->c,p2->c,p3->c));
    totvol += vol;
    /* barycenter */
  ...
  if ( totvol < EPSD )  return(0);
  totvol = 1.0 / totvol;
  ppt0->c[0] *= totvol;
  ppt0->c[1] *= totvol;
  ppt0->c[2] *= totvol;

  /* Check new position validity, and abort procedure when position invalid */
  calnew = DBL_MAX;
  for (k=0; k<ilist; k++) {
    iel = list[k] / 4;
    i0  = list[k] % 4;
...
int movbdyregpt(pMesh mesh,int *listv,int ilistv,int *lists,int ilists) {
  ...
  double          ll,m[2],uv[2],o[3],no[3],to[3],calold,calnew;
  ...
  for (l=0; l<ilists; l++) {
    ...
    calold = MG_MIN(calold,caleltsurf(mesh,&tt));
    for( i=0 ; i<3 ; i++ )
      if ( tt.v[i] == n0 )  break;
    assert(i<3);
    tt.v[i] = 0;
    calnew = MG_MIN(calnew,caleltsurf(mesh,&tt));
  }
  ...
  for (l=0; l<ilistv; l++) {
    ...
    calold = MG_MIN(calold,orcal(mesh,k));
    calnew = MG_MIN(calnew,orcal(mesh,0));
  }
  if ( calold < NULKAL && calnew <= calold )  return(0);
...
int movbdyrefpt(pMesh mesh, int *listv, int ilistv, int *lists, int ilists){
  ...
  Tria            tt;
  pxPoint         pxp;
  double          step,ll1old,ll2old,o[3],no[3],to[3],calold,calnew;
  ...
  if(!(ip1 && ip2 && (ip1 != ip2))){
    printf("ip1 : %d , ip2 %d pour le point %d \n",ip1,ip2,ip0);
  }
  ...
  if(!(BezierRef(mesh,ip0,ip,step,o,no,to)))
    return(0);
...
int movbdyridpt(pMesh mesh, int *listv, int ilistv, int *lists, int ilists){
  ...
  Tria            tt;
  pxPoint         pxp;
  double          step,ll1old,ll2old,o[3],no1[3],no2[3],to[3],calold,calnew;
  int             l,iel,ip0,ipa,ipb,iptmpa,iptmpb,it1,it2,ip1,ip2,ip,ref,nxp;
  unsigned char   i,i0,ie,iface,iface1,iface2,iea,ieb,ie1,ie2;
  char            tag;  
