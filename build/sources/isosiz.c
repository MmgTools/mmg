...
static double defsizreg(pMesh mesh,int nump,int *lists,int ilists) {
  pTetra       pt;
  pPoint       p0,p1;
  Tria         tt;
  Bezier       b;
  int          k,na,nb,ntempa,ntempb,iel,ier;
...
int defsiz_iso(pMesh mesh,pSol met) {
  ...
  if ( info.iso )  return(1);

  /* take input metric into account only if metric file given */
  for (k=1; k<=mesh->np; k++) {
    p0 = &mesh->point[k];
    if ( MG_VOK(p0) ) {
